...
class Trainer(Configurable):
    ...
    def bn_update(
            self, dataset, feature_key, batch_dim=0,
            device=0 if torch.cuda.is_available() else 'cpu'
    ):
        ...
        momenta = {}
        self.model.apply(_reset_bn)
        self.model.apply(lambda module: _get_momenta(module, momenta))
        n = 0
        with torch.no_grad():
            for i, example in enumerate(dataset):
                ...
                example = pt.data.example_to_device(example, device)
                self.model(example)

                n += b

        self.model.apply(lambda module: _set_momenta(module, momenta))
